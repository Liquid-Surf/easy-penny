...
type CachedDataset = responseInterface<SolidDataset & WithResourceInfo, unknown> & { save: (dataset: SolidDataset & WithResourceInfo) => void };
...
export function useDataset (url: UrlString | null): CachedDataset | null {
  ...
  const update = useCallback(async (dataset: SolidDataset & WithResourceInfo) => {
    if (!resourceUrl) {
      return;
    }
    ...
    const savedData = await saveSolidDatasetAt(resourceUrl, dataset, { fetch: session.fetch });
